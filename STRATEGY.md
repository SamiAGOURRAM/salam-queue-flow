...
- **Phase C.5 — DONE (shared channel policy):** extracted the channel-selection decision into a pure core policy `selectNotifyRoute` (preferred channel + per-channel enabled flags + availability fallback). Web `ChannelRouter` now delegates the decision to it (keeping its contact fetching/PII decryption); the MCP handler uses it too (upgraded from its minimal phone→SMS rule). "Which channel for this patient" is now SSOT — web UI and agent route identically. Verified: core 38 tests (8 new policy tests), web 209, mcp 151.
